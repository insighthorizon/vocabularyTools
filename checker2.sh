...
for i in $(seq 1 $NLINES); do
    ...
    ITEM_INFO=`grep -nr -i --include="prep.txt" "^$ITEM"` #ignoring letter case, must look for words ending with = or / (otherwise subword problem - finds "full" in "fully" etc.), (^ -from start of line), look in files with prefix voc_En and suffix .txt
    if [ "$ITEM_INFO" = '' ]; then
	NEW_ITEMS+="$ITEM"
	NEW_ITEMS+=$'\n'
    else
