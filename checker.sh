...
for i in $(seq 1 $NLINES); do
    ...
    ITEM_INFO=`grep -nr -i --include="voc_En*\.txt" "$ITEM-"` #ignoring letter case, must look for words ending with - (otherwise subword problem - finds full in fully etc.), look in files with prefix voc_En and suffix .txt
    if [ "$ITEM_INFO" = '' ]; then
	NEW_ITEMS+="$ITEM"
	NEW_ITEMS+=$'\n'
    else
